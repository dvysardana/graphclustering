...
class GraphClusterer_WI(object):
    ...
    def MKNN_worker_wrapper(self):
        ...
        self.MKNN_init()
        GraphClusterer_WI.configdata.logger.info("Initialization phase of GMKNN ends.")

        GraphClusterer_WI.configdata.logger.info("Working of G-MKNN begins")
        GraphClusterer_WI.configdata.logger.info("Running G-MKNN for different values of K.")

        K_range = list(range((int)(GraphClusterer_WI.configdata.K_min), (int)(GraphClusterer_WI.configdata.K_max)+1))

        for K in K_range:
            GraphClusterer_WI.configdata.logger.info("Running G-MKNN for the value of K: ")
            GraphClusterer_WI.configdata.logger.info(K)
    ...
    def MKNN_init(self):
        ...
        GraphClusterer_WI.configdata.do_config()
        GraphClusterer_WI.configdata.data_dir
        logger = GraphClusterer_WI.configdata.logger
        logger.debug("Debugging from inside MKNN_init method")

        #setting up the input matrices
        #Create SM and SM_orig
        GraphClusterer_WI.graphdata.create_SM_from_relfile(GraphClusterer_WI.configdata.inp_rel_file)

        #Expand SM
        GraphClusterer_WI.graphdata.setup_expanded_SM(GraphClusterer_WI.configdata.nhops, GraphClusterer_WI.configdata.inp_rel_file)

        #Create Edge Objects
        GraphClusterer_WI.graphdata.create_edge_objects()
    ...
    def MKNN_worker(self, K):
        self.phase1data = Phase1Data_WI(GraphClusterer_WI.graphdata,
                                     GraphClusterer_WI.configdata,
                                     K)
        self.MKNN_Phase1()
        self.phase2data = Phase2Data_WI(GraphClusterer_WI.graphdata,
                                     GraphClusterer_WI.configdata,
                                     K,
    ...
    def MKNN_Phase1(self):
        ...
        print('Num Nodes:')
        print((self.phase1data.graphdata.num_nodes))
        print('CI_list')
        self.helper.print_list(str(len(self.phase1data.cluster_initiator_list)))
