...
class CNodeData_WI_Gene(CNodeData_WI):
    def __init__(self, id, parent_id_1, parent_id_2, center, graphdata, configdata):
        super(CNodeData_WI_Gene, self).__init__(id, parent_id_1, parent_id_2, center, graphdata, configdata)
    ...
    def calculate_avg_func_gene_pair_sim(self):
        ...
        for i in range(0, len(node_list)):
            for j in range(0, len(node_list)):
                if i < j:
                    sumsim = sumsim + self.graphdata.SM_GO[node_list[i],node_list[j]]
                    if self.graphdata.SM_GO[node_list[i],node_list[j]] != 0:
                        count = count + 1

        if count != 0:
            avgsim = float(sumsim)/count
        else:
            avgsim = 0
