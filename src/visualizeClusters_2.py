...
def visualizeClusters_2(SM, CL_List, node_codes, num_clusters, K, phase, currentdate_str, dataset_name, eval_results_dir, log):

    #logger = log.get_logger(__name__)
    logger = logging.getLogger('root')
    logger.debug("Debugging from inside visualizeClusters_2 module")


    #phase = 2
    plt.close("all")
    #currentdate = datetime.datetime.now()
    filename = eval_results_dir + "/figures/phase" + str(phase) + "/" + currentdate_str + "_GMKNN_" + dataset_name + "_K_" + str(K) + "_Phase_" + str(phase) + "_V_NX.png"
    ...
    cluster_colors = generateCMap(CL_List_unique, num_clusters)
    ...
    for i in range(0,len(node_codes)):
        for j in range(0, len(node_codes)):
            if(SM[i,j] != -1 and SM[i,j] != 0 and i!=j): #change SM[i][i] please to some other value than 1.
