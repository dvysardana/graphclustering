...
class Phase2Data_WI_Gene(Phase2Data_WI):
    def __init__(self, graphdata, configdata, K, cnodes_dict, next_cluster_label, num_clusters):
        ...
        self.GO_SIM_TYPE = "BP" #GO term type used for calculating gene-gene functional similarity
                ...
                if i != self.next_cluster_label:
                    for j in range(i+1, self.next_cluster_label):
                        ...
                        if cnode_j.active == True:
                            ...
                            if(len(cnode_i.gene_GO_terms_dict[self.GO_SIM_TYPE]) != 0 and len(cnode_j.gene_GO_terms_dict[self.GO_SIM_TYPE]) != 0):
                                ...
                                self.c_SM_GO[cm_i][cm_j] = self.calculate_gene_sim((cnode_i.gene_GO_terms_dict[self.GO_SIM_TYPE], cnode_j.gene_GO_terms_dict[self.GO_SIM_TYPE]))
                            else:
    ...
    def calculate_goterm_golist_score(self, inp_goterm, inp_golist):
        ...
        for goterm in inp_golist:
            if goterm in self.graphdata.GO_codes and inp_goterm in self.graphdata.GO_codes:
    ...
    def filter_clusters(self):
        ...
        for (cnode_id, func_sim) in self.cnode_functional_sort_list:
            ...
            target1.write(" ")
            target1.write(str(self.cnodes_dict[cnode_id].mean_edges))
            target1.write(" ")
            target1.write(str(self.cnodes_dict[cnode_id].standard_deviation_edges))
            target1.write(" ")
            target1.write(str(self.cnodes_dict[cnode_id].cohesion))
            target1.write(" ")
            target1.write(str(func_sim))
            target1.write(" ")
            target1.write(str(self.cnodes_dict[cnode_id].cnode_CP_status))
            target1.write("\n")
